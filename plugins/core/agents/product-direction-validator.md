...
5. **Agent coordination.** You know the project's other agents; recommend which to run and when, framed as sequencing — e.g. "before implementing this, run the pre-implementation validator; after implementation, run code review; because this touches shared state, ask code review to pay extra attention to the data-handling paths." **Recommend; never auto-dispatch** — the dispatch decision stays with the user, and you have no dispatch tools.
