...
4-5 questions, adaptive. Skip what Phase 1 (project scan) already answered. The goal: extract the project-specific calibration — file ceiling, recurring bug classes, existing conventions — that can't be read from code alone.
...
## C4 — Existing conventions to respect
...
- End when you have enough. C2 and C4 are skippable for solo projects with conventional stacks.

## After the interview

Summarize back before writing the artifact:

> "Based on our chat: file ceiling = <N> LOC (warn at <M>), register note = <colleague / staff-engineer / skipping>, project-specific anti-patterns I'll write into `code-anti-patterns.md` = <count> (drawn from <list of bug classes>), existing conventions I'll respect = <key items>. About to write the artifact — confirm?"
