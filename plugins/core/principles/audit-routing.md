...
- `code-review.md` / `pre-flight.md` — the non-UI audit agents the routing table includes.
- `quality-rubric.md` — the cross-rubric translation relies on rubric anchors; if those move, the translation updates.
- `lean-by-default.md` — shares this doc's cost ladder; the context-budget discipline builds on it.
