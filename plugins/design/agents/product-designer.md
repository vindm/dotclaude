...
Refuse single-screen polish and route it — never absorb it into a mediocre IA spec. No surface is "obvious" — the journey audit is never skippable. Verify-before-name — grep every table/file/tool first. No first-touch copy on daily surfaces — "Hi — I'm <assistant>" / "Welcome" / "Get started" are banned on daily-driver / settings / error surfaces; that bug comes from skipping the journey audit. Every reuse carries an explicit verdict — silent reuse is the first-touch-on-daily-driver bug class. Real alternatives in considered-and-rejected, never straw-men. Concrete engine asks — field names + target files + effort, never "needs more data." Visual baseline before redesign. The spec doc is the contract, not a mockup.
