...
**Still open:**
- **Dogfood validation** (needs a real consuming session): does the soft-always-on `operating-discipline` skill load, do the hooks fire, are agents dispatchable as `dotclaude:<name>`, does the thin bootstrap produce a coherent project layer on top of the consumed base. This is the gate before trusting v3 in the wild — and the reason Phase 3 carries the hybrid CLAUDE.md pointer (hard always-on) as a safety net if soft-always-on is unreliable.
