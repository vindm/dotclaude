...
- **Shed local dupes** — a consuming project that adopts the base drops its now-redundant local universals (its own `handoff`, `code-review`, universal hooks, the "how you work" section of its `CLAUDE.md` → replaced by a one-line pointer to `dotclaude:operating-discipline`).
